          ...
          <v-card elevation="2" class="rounded-xl flex-grow-1 d-flex flex-column overflow-hidden bg-white" style="height: 100%; max-height: 100%; min-height: 0;">
            ...
            <v-card-title class="bg-white pa-3 border-b d-flex align-center justify-space-between flex-shrink-0" style="border-bottom: 1px solid #e8eaed;">
              <div class="d-flex align-center">
                ...
                <div>
                  <div class="text-subtitle-2 font-weight-bold" style="color: #202124; line-height: 1.2;">
              ...
              <div class="d-flex align-center">
                ...
                >
                  <v-btn value="overview" size="small" class="text-capitalize font-weight-bold">
                    <v-icon start size="15">mdi-view-dashboard-outline</v-icon> Panorama General
                  </v-btn>
                  <v-btn value="deepdive" size="small" class="text-capitalize font-weight-bold">
                    <v-icon start size="15">mdi-bullseye-arrow</v-icon> Deep Dive: Deportes
                  </v-btn>
                </v-btn-toggle>
              </div>
            </v-card-title>
            ...
            <v-card-text class="flex-grow-1 pa-3 overflow-y-auto custom-scrollbar" style="background-color: #f8f9fa; min-height: 0;">
              ...
              <div v-show="retailTab === 'overview'">
                ...
                <v-row class="mb-2" dense>
                  <v-col cols="6" sm="3">
                    <v-card class="rounded-lg pa-2 text-center bg-white" elevation="1" style="border-top: 3px solid #4285F4;">
                      <div class="text-caption text-grey-darken-1 text-uppercase font-weight-bold" style="font-size: 10.5px;">Ventas QTD</div>
                  ...
                  <v-col cols="6" sm="3">
                    <v-card class="rounded-lg pa-2 text-center bg-white" elevation="1" style="border-top: 3px solid #1967d2;">
                      <div class="text-caption text-grey-darken-1 text-uppercase font-weight-bold" style="font-size: 10.5px;">Ticket Prom.</div>
                      <div class="text-subtitle-1 font-weight-bold mt-1" style="color: #1967d2;">$1,740</div>
                      <v-chip size="x-small" color="info" class="font-weight-bold mt-1" variant="flat">
                  ...
                  <v-col cols="6" sm="3">
                    <v-card class="rounded-lg pa-2 text-center bg-white" elevation="1" style="border-top: 3px solid #34A853;">
                      <div class="text-caption text-grey-darken-1 text-uppercase font-weight-bold" style="font-size: 10.5px;">Transacciones</div>
                      <div class="text-subtitle-1 font-weight-bold mt-1" style="color: #34A853;">138,450</div>
                      <span class="text-caption font-weight-bold" style="color: #5f6368; font-size: 10px;">Conv. 3.5%</span>
                    </v-card>
                  </v-col>

                  <v-col cols="6" sm="3">
                    <v-card class="rounded-lg pa-2 text-center bg-white" elevation="1" style="border-top: 3px solid #EA4335;">
                      <div class="text-caption text-grey-darken-1 text-uppercase font-weight-bold" style="font-size: 10.5px;">Stock en Riesgo</div>
                ...
                <v-card class="mb-2 rounded-lg pa-3 bg-white" elevation="1">
                  <div class="d-flex align-center justify-space-between mb-1">
                    <div>
                      <span class="font-weight-bold text-caption text-uppercase" style="color: #202124;">
                        <v-icon size="14" color="#4285F4" class="mr-1">mdi-chart-bar</v-icon> Ventas Netas por Categoría ($M MXN)
                      </span>
                    </div>
                    <span class="text-caption text-grey-darken-1" style="font-size: 10.5px;">12 Macro-Sectores Consolidado Nacional</span>
                  </div>
                ...
                <v-card class="mb-2 rounded-lg overflow-hidden bg-white" elevation="1" style="border: 1px solid #e8eaed;">
                  <div class="pa-2 px-3 d-flex align-center justify-space-between" style="background-color: #fafafa; border-bottom: 1px solid #e8eaed;">
                    <span class="font-weight-bold text-caption text-uppercase" style="color: #202124;">
                    ...
                    <table class="retail-matrix-table">
                      <thead>
                        <tr>
                          ...
                          <th style="text-align: right; min-width: 90px;">Ventas ($ MXN)</th>
                          <th style="text-align: right; min-width: 80px;">Ticket Prom.</th>
                          <th style="text-align: center; min-width: 70px;">YoY (%)</th>
                          <th style="min-width: 180px;">Google Trends / Demanda Externa</th>
                          <th style="text-align: center; min-width: 90px;">Acción Agente</th>
                        </tr>
                      </thead>
                      <tbody>
                        <tr 
                          ...
                          :key="cat.name" 
                          :style="cat.highlight ? 'background-color: #fff3f0; border-left: 4px solid #EA4335;' : 'border-bottom: 1px solid #f0f0f0;'"
                        >
                          <td>
                            <div class="d-flex align-center">
                              <v-icon :color="cat.iconColor || '#4285F4'" size="16" class="mr-1">{{ cat.icon }}</v-icon>
                              <span class="font-weight-bold" style="color: #202124;">{{ cat.name }}</span>
                              <v-chip v-if="cat.badge" size="x-small" :color="cat.badgeColor || 'primary'" class="ml-1 font-weight-bold" variant="flat" style="height: 17px; font-size: 9px;">{{ cat.badge }}</v-chip>
                            </div>
                          </td>
                          <td style="text-align: right; font-weight: 700; color: #202124;">{{ cat.sales }}</td>
                          <td style="text-align: right; color: #3c4043; font-weight: 500;">{{ cat.ticket }}</td>
                          <td style="text-align: center;">
                            <v-chip size="x-small" :color="cat.growth > 0 ? 'success' : 'error'" variant="flat" class="font-weight-bold" style="height: 18px; font-size: 10px;">
                              {{ cat.growth > 0 ? '+' : '' }}{{ cat.growth }}%
                            </v-chip>
                          </td>
                          <td>
                            <span :style="{ color: cat.trendColor || '#5f6368', fontWeight: 600, fontSize: '11px' }">
                              <v-icon size="13" :color="cat.trendColor || '#5f6368'">{{ cat.trendIcon || 'mdi-trending-neutral' }}</v-icon> {{ cat.trendText }}
                            </span>
                          </td>
                          <td style="text-align: center;">
                            <v-btn 
                              size="x-small" 
                              variant="flat" 
                              :color="cat.highlight ? '#EA4335' : '#4285F4'" 
                              rounded="pill" 
                ...
                <v-row dense class="mt-1">
                  <v-col cols="12" sm="6">
                    <v-card class="rounded-lg pa-2 px-3 bg-white cursor-pointer transition-swing" elevation="1" style="border-left: 4px solid #EA4335;" @click="setRetailTab('deepdive')">
                  ...
                  <v-col cols="12" sm="6">
                    <v-card class="rounded-lg pa-2 px-3 bg-white cursor-pointer" elevation="1" style="border-left: 4px solid #4285F4;" @click="askRetailPrompt('Analiza la oportunidad de margen en Electrónica & Gaming con ticket promedio de $4,800 MXN')">
                      <div class="d-flex align-center justify-space-between">
                        <span class="text-caption font-weight-bold text-uppercase" style="color: #4285F4; font-size: 11px;">💡 Oportunidad de Margen</span>
                        <v-icon color="#4285F4" size="16">mdi-trending-up</v-icon>
                      </div>
                      <div class="text-caption font-weight-bold mt-1" style="color: #202124;">Electrónica & Gaming (+14.2%)</div>
                      <p class="text-caption text-grey-darken-2 mb-0" style="font-size: 11px; line-height: 1.35;">
                        Ticket promedio en <strong>$4,800 MXN</strong> impulsado por lanzamientos y Back-to-School.
                      </p>
                    </v-card>
                  </v-col>
                </v-row>
              ...
              <div v-show="retailTab === 'deepdive'">
                ...
                <v-row class="mb-2" dense>
                  <v-col cols="4">
                    <v-card class="pa-2 rounded-lg text-center bg-white" elevation="1" style="border-top: 3px solid #EA4335;">
                  ...
                  <v-col cols="4">
                    <v-card class="pa-2 rounded-lg text-center bg-white" elevation="1" style="border-top: 3px solid #34A853;">
                  ...
                  <v-col cols="4">
                    <v-card class="pa-2 rounded-lg text-center bg-white" elevation="1" style="border-top: 3px solid #FBBC05;">
                ...
                <v-card class="pa-3 rounded-lg mb-3 bg-white" elevation="1">
                  <div class="d-flex align-center justify-space-between mb-1">
                    <div>
                ...
                <v-card class="pa-3 rounded-lg bg-blue-grey-lighten-5" elevation="0" style="border-left: 4px solid #4285F4;">
                  <div class="font-weight-bold text-caption" style="color: #1967d2;">🎯 Oportunidad de Negocio Identificada por el Agente:</div>
  ...
  methods: {
    ...
    initRetailCategoryChart() {
      this.$nextTick(() => {
        ...
        const colors = categories.map(c => (c && c.iconColor) ? c.iconColor : '#4285F4');
